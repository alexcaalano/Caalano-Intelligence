...
## v3.681.0 - 2026-09-16 (commit 4f9220a)
...
## v3.666.0 - 2026-09-15 (commit 08a4b7b)
...
**Monthly Report: one side panel, on the right** (commit f9e34c3)
...
- The Caalano360 tab's Channel performance table replaces ROI by channel, and its Key event reach bars (channel split per event, previous-period tick, the biggest leak flagged) replace the cohort funnel. Both come from the drill frozen with the report; a snapshot generated before this keeps the funnel it had.
- The revenue matrix (status change vs created on) and the status donut stay, side by side at the bottom. The Channel performance table is one component now, shared with the Caalano360 tab.
...
- The page can be switched off per role or person under Settings → Access → Visibility like any other sidebar entry. The Deals & Actions tab inside a client's workspace is unchanged.

---

## v3.648.0 - 2026-09-14

**Trend Report on a phone** (commit 2ea69f4)
...
**Ad-set rules in the Monthly Report and the creative cards** (commit 269130e)
...
- The key-event people feed now sends the no-show flag per calendar, so the
  ...
  confirmed; a real no-show reads as No-show. Follows v3.514.0.

 · `b9ab2b6` - The people drill names unresulted appointments
