...
- **Agency new-business pipeline** — pulled from **Caalano Systems / Caalano Systems**.
...
The connectors (Reporting Ninja, Caalano Systems) are authorised to the Claude
...
4. Caalano Systems: `get-pipelines` + `search-opportunity` (status=open) on the Lead Connector.
...
**Reporting Ninja REST API** and **Caalano Systems API** on each page load, using
...
## Scope note — Caalano Systems / Caalano Systems
The Lead Connector is currently authorised for a **single** Caalano Systems sub-account
(Caalano Digital's own HQ pipeline). To pull **every client's** Caalano Systems pipeline
