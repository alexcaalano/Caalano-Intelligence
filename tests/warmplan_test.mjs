...
ok(has(/scope=speed.*bhDays=1%2C2%2C3%2C4%2C5.*bhStart=540/), 'speed carries the default business hours the app sends')
ok(ranges.mtd && ranges.mtd.from.endsWith('-01') && ranges.mtd.to >= ranges.mtd.from, 'a month-to-date window')
ok(has(new RegExp(`scope=saleshub.*from=${ranges.mtd.from}.*to=${ranges.mtd.to}.*preset=this_month.*stale=7.*bhDays=`)), 'the Sales Hub, month to date, in the shape the hub requests')
